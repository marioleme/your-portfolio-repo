...
export const useGitHubData = (username) => {
  ...
  const fetchGitHubData = useCallback(async () => {
    ...
    if (hasAttempted.current) {
      setLoading(false);
      // Use fallback data if already attempted and failed
      if (!data) {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        setError('GitHub API rate limit reached. Displaying fallback data.');
        console.info('Using fallback data due to rate limiting');
      ...
      // Always set fallback data when API fails
      setData(getFallbackData());
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (username && !hasAttempted.current) {
      fetchGitHubData();
    } else if (username && hasAttempted.current && !data) {
      // Ensure fallback data is set if no data exists
      setData(getFallbackData());
      setLoading(false);
    }
  ...
  useEffect(() => {
    const fetchRepositories = async () => {
      ...
      try {
        ...
        // Handle specific error types
        if (err.response?.status === 403) {
          setError('GitHub API rate limit reached. Displaying fallback repositories.');
          console.info('Using fallback repositories due to rate limiting');
        } else if (err.response?.status === 404) {
          setError('GitHub user not found.');
        } else {
    ...
    if (username && !hasAttempted.current) {
      fetchRepositories();
    } else if (username && hasAttempted.current && data.length === 0) {
      // Ensure fallback data is set if no data exists
      setData(getFallbackRepos());
      setLoading(false);
    }
